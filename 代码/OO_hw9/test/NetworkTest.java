...
public class NetworkTest {
    ...
    public void testInitialTripleSumZero() {
        assertEquals("Initial triple sum should be zero", 0, network.queryTripleSum());
    }
    ...
    public void testSingleTriangleFormation() throws Exception {
        ...
        // No triangle yet
        assertEquals("No triangle formed before closing edge", 0, network.queryTripleSum());
        network.addRelation(3, 1, 1);
        // One triangle
        assertEquals("One triangle should be formed", 1, network.queryTripleSum());
    }
    ...
    public void testMultipleTrianglesCompleteGraph() throws Exception {
        ...
        // K4 has 4 triangles
        assertEquals("Complete graph K4 should have 4 triangles", 4, network.queryTripleSum());
    }
    ...
    public void testAddEdgeCreatesTwoTriangles() throws Exception {
        ...
        network.addRelation(2, 4, 1);
        network.addRelation(3, 4, 1);
        assertEquals("No triangles in a square without diagonals", 0, network.queryTripleSum());
        // Add diagonal between 1 and 4: should create two triangles (1-2-4 and 1-3-4)
        network.addRelation(1, 4, 1);
        assertEquals("Adding one edge should form two triangles", 2, network.queryTripleSum());
    }
    ...
    public void testEdgeRemovalDecreasesTriangles() throws Exception {
        ...
        network.addRelation(2, 3, 1);
        network.addRelation(3, 1, 1);
        assertEquals("One triangle formed", 1, network.queryTripleSum());
        // Remove edge by decreasing weight to zero
        network.modifyRelation(1, 2, - 1);
        assertEquals("Triangle count should decrease to zero after edge removal", 0, network.queryTripleSum());
    }
}
