    ...
    public void testExtractLoopPath() {
        ...
        for (int i = 0; i < path.size(); i++) {
            for (int j = i + 1; j < path.size(); j++) {
                if (path.getNode(i) == path.getNode(j)) {
                    flag = true;
                }
            }
        }
        ...
        if (! flag) {
             ...
             */
            assertTrue(path.extractLoopPath() instanceof Path);
            Path loopPath = path.extractLoopPath();
            /*
            ensures (\exists int i; 0 <= i && i < nodes.length-1;
            ...
            for (int i = 0; i < path.size() - 1; i++) {
                ...
                for (int j = 0; j < loopPath.size(); j++) {
                    if (((i + j) < path.size()) && (loopPath.getNode(j) == path.getNode(i + j))) {
                        continue;
                    } else {
                        fl_j = false;
                        break;
             ...
             */
            assertEquals(loopPath.getNode(0), loopPath.getNode(loopPath.size() - 1));
        }
        //TODO:[3]
    }
