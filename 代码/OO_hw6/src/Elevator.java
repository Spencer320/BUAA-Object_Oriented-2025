...
public class Elevator extends Thread {
    ...
    enum OutputType {
        OPEN, CLOSE, ARRIVE, IN, OUT_F, OUT_S, BEGIN, END
    }
    ...
    public int getWaitingPassengerSize() {
        return waitingPassenger.getPassengerSet().size();
    }
    ...
    public void run() {
        while (true) {
            // System.out.println("Elevator " + this.elevatorId + " is waking...");
            while (! hasTasks() && ! waitingPassenger.isEnd() && ! isSche()) {
            ...
            if (! hasTasks() && waitingPassenger.isEnd() && ! isSche()) {
                // System.out.println("Elevator " + this.elevatorId + " has ended.");
                break;
            }
    ...
    private void handleSchedulingState() {
        ...
        int speed = waitingPassenger.getScheSpeed();
    ...
    public void personOut(Passenger passenger) {
        ...
        if (currentFloor == passenger.getToFloor()) {
            formatOutput(OutputType.OUT_S, passenger);
        } else {
            formatOutput(OutputType.OUT_F, passenger);
        }
    }
    ...
    public void formatOutput(OutputType type) {
        switch (type) {
            ...
            case ARRIVE:
                ...
                break;
            case BEGIN:
                TimableOutput.println("SCHE-BEGIN-" + getElevatorId());
                break;
            case END:
                TimableOutput.println("SCHE-END-" + getElevatorId());
                break;
            default:
        }
    }
        ...
        synchronized (currentPassenger) {
            for (Passenger passenger : currentPassenger.getPassengerSet()) {
                ElevatorRequest request = new ElevatorRequest(new Passenger(
                    passenger.getId(), currentFloor, passenger.getToFloor(), passenger.getPriority()
                ));
                requestTable.offer(request);
                copy.add(passenger);
            }
